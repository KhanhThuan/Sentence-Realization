...
public class Properties {
	...
	public List<String> realizeSentences(int numberTrainSentences, String testFile){
		Properties.getProperties().setSmooth(Properties.ADD_ONE);
      ...
      //  int pivot = (int)(corpara.size() * 0.1);
		...
		try {
			...
			//	nTest += incsSize;
				...
				for(int i =0; i < testSentences.size(); i++) results.add(testSentences.get(i).getOptimizedSequence());
				return results;
	...
	public List<String> testRealizeSentences( String testFile){
		try{
			/*******************************Testing**********************************/
			System.out.println("Training Model finished.");
			System.out.println("Testing.....");
			DependencyTree tree = Properties.getProperties().getTree();
				DependencyTree testTree = new DependencyTree();
				 ModelBuilder.makeTestSentences(testFile, testTree,new int[]{2,3}, 3);
				List<TreeNode> testSentences=testTree.getSentences();
			//	nTest += incsSize;
	...
	public void trainRealizeSentences(int numberTrainSentences){
		Properties.getProperties().setSmooth(Properties.ADD_ONE);
      ...
      //  int pivot = (int)(corpara.size() * 0.1);
		...
		try {
			...
			Properties.getProperties().setLambda(/*optimizedLambda*/ lamdas);
			Properties.getProperties().setTree(tree);
	...
	public void setTree(DependencyTree tree) {
		Properties.tree = tree;
	}
	public VnDependencyParsing getDp() {
		return dp;
	}
	public void setDp(VnDependencyParsing dp) {
		Properties.dp = dp;
	}
}
